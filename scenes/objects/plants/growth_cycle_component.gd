...
func growth_states(start_day: int, curr_day: int) -> void:
	...
	var current_growth_state_name = DataTypes.GrowthStates.keys()[current_growth_state]
	#print("Current growth state: ", current_growth_state_name, " State index: ", state_index)
...
func harvest_state(start_day: int, curr_day: int) -> void:
	...
	var days_passed = (curr_day - start_day) % days_until_harvest
