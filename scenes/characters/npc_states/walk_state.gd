...
func _ready() -> void:
	navigation_agent_2d.velocity_computed.connect(set_safe_velocity)
...
func set_safe_velocity(safe_velocity: Vector2) -> void:
	animated_sprite_2d.flip_h = safe_velocity.x < 0	
	character.velocity = safe_velocity
	character.move_and_slide()

func _on_next_transitions() -> void:
	if character.current_cycle == character.walk_cycles:
		character.velocity = Vector2.ZERO
		transition.emit("idle")

func _on_enter() -> void:
	animated_sprite_2d.play("walk")
	character.current_cycle = 0
